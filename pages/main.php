...
if ($func == 'setstatus') {
    ...
    $query = "SELECT `term`, `active` FROM rex_glossar WHERE `pid` ='" . addslashes($oid) . "' ";
    $sql->setQuery($query);
    if ($sql->getRows() == 1) {
      $term = $sql->getValue('term');
      $query = "UPDATE  rex_glossar  SET `active` = '$status' WHERE `pid` ='" . addslashes($oid) . "' ";
      $sql->setQuery($query);
    }
...
if ($func == '') {
   ...
   //  \rex_extension::register('REX_FORM_CONTROL_FIELDS', '\Glossar\Extension::glossarFormControlElement');

    $form = rex_form::factory(rex::getTable('glossar'), '', 'pid = ' . $pid,'post', false);
    ...
    /*
        ...
        $field->setLabel($this->i18n('glossar_label_active'));
